...
public class TestAuthController {
     ...
     * @return 액세스 토큰
     */
    @Operation(summary = "테스트용 토큰 생성 (장애인)", description = """
            ...
            **주의:**
            ...
            """)
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "토큰 생성 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TokenResponse.class))),
            @ApiResponse(responseCode = "404", description = "테스트 회원을 찾을 수 없음 (DB에 memberId=100인 회원이 없음)", content = @Content(mediaType = "application/json")),
     ...
     * @return 액세스 토큰
     */
    @Operation(summary = "테스트용 토큰 생성 (도우미)", description = """
            테스트를 위한 임시 JWT 액세스 토큰을 생성합니다.
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "토큰 생성 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TokenResponse.class))),
            @ApiResponse(responseCode = "404", description = "테스트 회원을 찾을 수 없음 (DB에 memberId=700인 회원이 없음)", content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "500", description = "서버 내부 오류", content = @Content(mediaType = "application/json"))
    })
