...
public class ProfileInfoResDTO {
        ...
        @Schema(description = "장애 유형명 (장애인 전용)", example = "시각장애")
        private String disabilityTypeNm;
        ...
        public static ProfileInfoResDTO from(GetProfileInfoUseCase.Result result) {
                return new ProfileInfoResDTO(
                                ...
                                result.getHoney(),
                                result.getReviews() != null ? result.getReviews().stream()
                                                .map(r -> new ReviewKeywordDTO(r.keywordId(), r.count()))
                                                .toList() : null,
                                result.getBadges() != null ? result.getBadges().stream()
                                                .map(BadgeStatusDTO::from)
                                                .toList() : null,
                                result.getDocuments() != null ? result.getDocuments().stream()
