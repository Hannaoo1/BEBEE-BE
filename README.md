...
### :feet: 리뷰 API
- 도우미 ↔ 의뢰인 양방향 리뷰 시스템 설계 및 구현
- 키워드 기반 리뷰 작성 (긍정 / 부정 카테고리)
  
### :feet: 뱃지 API
- 장애 유형 누적 기반 뱃지 자동 부여
- 5회 → 조력자, 10회 → 전문가 등급 시스템
  
### :feet: 활동 완료 API
- 도우미 / 의뢰인 완료 버튼 클릭 케이스별 처리(로직 구현)
  - 둘 다 클릭 → 즉시 완료
  - 의뢰인만 클릭 → 즉시 완료
  - 도우미만 클릭 → 3일 후 스케줄러 처리
  - 둘 다 미클릭 → 2일 후 경고 알림, 3일 후 미완료 처리
    
### :feet: 이벤트 처리
- 활동 완료 시 의뢰인 장애 유형에 따른 뱃지 생성 이벤트 발행
- 활동 완료 시 리뷰 요청 이벤트 발행
